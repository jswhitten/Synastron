...
# Parameters
distance_threshold = 10  # Change this value to control the number of clusters
magnitude_threshold = 8  # Magnitude threshold for applying the penalty
magnitude_penalty_factor = 0.5  # Penalty factor to be added to the distance when the magnitude threshold is exceeded
method = 'single'

def main(method, distance_threshold, magnitude_threshold, magnitude_penalty_factor):
  ...
  query = "SELECT x, y, z, iauname, altname, bf, gl, absmag, dist FROM hyg WHERE dist < 35"
  ...
  for i in range(df.shape[0]):
    for j in range(i + 1, df.shape[0]):
      maxmag = max(df.iloc[i]['absmag'], df.iloc[j][
          'absmag'])
      if maxmag > magnitude_threshold:
        dist_matrix[i, j] += (maxmag - magnitude_threshold) * magnitude_penalty_factor
        dist_matrix[j, i] = dist_matrix[i, j]
  
  # Perform hierarchical clustering
  # allowed methods: 'single', 'complete', 'average', 'weighted', 'centroid', 'median', 'ward'
  condensed_dist_matrix = squareform(dist_matrix, checks=False)
  clusters = linkage(condensed_dist_matrix, method=method)
  ...
  # Filter out the clusters that don't have at least one star within 32.6 light years
  filtered_labels = []
  for label in set(labels):
      cluster_indices = np.where(labels == label)[0]
      cluster_data = df.iloc[cluster_indices].copy()  # Add .copy() here to avoid SettingWithCopyWarning
      if (cluster_data['dist'] <= 25).any():
  ...
  for label in filtered_labels:
    ...
    marker_sizes = normalize_marker_size(1 / cluster_data['absmag'])
    ...
    # Draw lines between stars in the same cluster
    line_x = []
    line_y = []
    line_z = []
  ...
  with open(f'output/clusters_{method}_{distance_threshold}.json', 'w') as json_file:
    json.dump(json_clusters, json_file, indent=2)
  
  # Save the plot to an HTML file
  plot_file = f'output/plot_{method}_{distance_threshold}.html'
...
if __name__ == "__main__":
    main(method, distance_threshold, magnitude_threshold, magnitude_penalty_factor)